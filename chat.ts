...
while (true) {
    ...
    if (query.toLowerCase() === "exit") {
        const confirmation = readlineSync.question("Ella: Are you sure you want to exit the chat? (y/n): ");
        ...
        console.log("0. Quit");
        const userInput = readlineSync.question("Ella: Please enter the corresponding number: ");

        if (userInput === "1") {
            const { exec } = require('child_process');
            exec("python smartcontract_build.py", (error: any, stdout: string, stderr: string) => {
                if (error) {
